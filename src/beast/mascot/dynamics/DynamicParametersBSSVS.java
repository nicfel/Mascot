...
public class DynamicParametersBSSVS extends Dynamics implements Loggable {	
...
//    		"forwardsMigration", "input of backwards in time migration rates", Validate.REQUIRED);  
    ...
    public Input<List<EffectivePopulationSizeDynamics>> migrationInput = new Input<>(
    		"migrationDynamics", "input of the log forward migration rates", new ArrayList<>());    
...
//	RealParameter migration;
    ...
    public void initAndValidate() {
    	...
    	if (dimensionInput.get()*(dimensionInput.get()-1)!=migrationInput.get().size() || fromBeautiInput.get()){
    		throw new IllegalArgumentException("the number of migraiton dynamics should be " + (dimensionInput.get()*(dimensionInput.get()-1)));
    	}
		...
		if (indicatorInput.get().getDimension() != migrationInput.get().size())
			indicatorInput.get().setDimension(migrationInput.get().size());
			...
			for (int b = 0; b < dim; b++){
				if (a!=b){
    				if (indicatorInput.get().getArrayValue(dirs[b][a])>0.5) {
						m[a * dim + b] = Math.min(maxRateInput.get(), NeInt[b]*migrationInput.get().get(dirs[b][a]).getNeTime(t)/NeInt[a]);
//    					System.out.println(migrationInput.get().get(dirs[b][a]).getNeTime(t));
//    					m[a * dim + b] = migrationInput.get().get(dirs[b][a]).getNeTime(t);
    				}else {
						m[a * dim + b] = 0.0;
    				}
				}
...
//    	return m;  	
	...
	public boolean intervalIsDirty(int j) {
		...
		for (int i = 0; i < migrationInput.get().size(); i++)
			if(migrationInput.get().get(i).isDirty())
				return true;
	...
	public void init(PrintStream out) {
		if (isForward){
			...
			for (int a = 0; a < dimensionInput.get(); a++)
				for (int b = 0; b < dimensionInput.get(); b++)
					if (a!=b) {
						for (int c = 0; c < intTimes.length; c++) {
							out.print(String.format("mig_%s_to_%s.%d\t", getStringStateValue(a), getStringStateValue(b), c));
						}
					}
			}			
	}
				...
				for (int b = 0; b < dimensionInput.get(); b++) { 
					if (a!=b) {
						for (int c = 0; c < intTimes.length; c++) {
							...
							double t = rateShiftsInput.get().getIntervalMidpoint(c);

							if (indicatorInput.get().getArrayValue(dirs[a][b])>0.5)
								out.print(migrationInput.get().get(dirs[b][a]).getNeTime(t) + "\t");
							else
								out.print(0.0 + "\t");
						}
					}
