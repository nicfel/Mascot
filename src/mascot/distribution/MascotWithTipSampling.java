...
public class MascotWithTipSampling extends StructuredTreeDistribution {
...
//    	tree = treeInput.get();
    	...
    	if (dynamics.typeTraitInput.get() != null) {
    		...
    		for (int i = 0; i < tree.getLeafNodeCount(); i++) {
				if (tipsToSample.contains(tree.getNode(i).getID())) {
					nodeType[i]=-1 - tipsToSample.indexOf(tree.getNode(i).getID());
					totTipsFound++;
				}else {
...
//					System.exit(1);
		...
		if (totTipsFound != tipsToSample.size() && tipsToSample.size() > 0) {
			throw new IllegalArgumentException("Not all tips that should be sampled found in tree: " + tipsToSample);
		}
